...
CFieldFire::CFieldFire(LPDIRECT3DDEVICE9 pDevice)
:CParticleObject(pDevice)
{

}

CFieldFire::~CFieldFire()
{

}

CFieldFire* CFieldFire::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CFieldFire*		pFieldFire = new CFieldFire(pDevice);

	if(FAILED(pFieldFire->Initialize()))
		ENGINE::Safe_Delete(pFieldFire);

	return pFieldFire;
}

HRESULT CFieldFire::Initialize(void)
{
	m_PrtN	= 10;
...
void CFieldFire::Update(void)
{
	...
	for(i=0; i<m_PrtN; ++i)
	{
		...
		pPrt->dColor	=	D3DXCOLOR(0.7f, 0.3f, 0.f, 1.f);
...
D3DXMATRIX* CFieldFire::GetWorldMatrix(void)
{
	return NULL;
}

D3DXMATRIX* CFieldFire::GetViewMatrix(void)
{
	return NULL;
}

D3DXMATRIX* CFieldFire::GetProjMatrix(void)
{
	return NULL;
}

LPDIRECT3DDEVICE9* CFieldFire::GetDevice(void)
{
	return NULL;
}

void CFieldFire::SetAni(BOOL bAni/*=TRUE*/)
{
	m_bAni = bAni;
...
void CFieldFire::SetAvgTime(FLOAT fTime)
{
	m_fTimeAvg = fTime;
}

void CFieldFire::Release(void)
{

}

void CFieldFire::SetPart(int nIdx)
{
	...
	// 입자의 표현 요소
	pPrt->PrsW = 5.f + rand()%10;
	pPrt->PrsW *= 0.03f;

	pPrt->PrsH = 5.f + rand()%10;
	pPrt->PrsH *= 0.03f;
