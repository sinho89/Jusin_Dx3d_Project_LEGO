...
CPlayerGround::CPlayerGround(LPDIRECT3DDEVICE9 pDevice)
: CRcObject(pDevice)
{

}

CPlayerGround::~CPlayerGround(void)
{
	Release();
}


HRESULT CPlayerGround::Initialize(void)
{
...
HRESULT CPlayerGround::AddComponent(void)
{
	...
	// Texture
	pComponent = m_pResourcesMgr->CloneResource(ENGINE::RESOURCE_DYNAMIC, L"Texture PlayerGround");
	m_pTexture = dynamic_cast<ENGINE::CTexture*>(pComponent);
	NULL_CHECK_RETURN(m_pTexture, E_FAIL);
	m_mapComponent.insert(MAPCOMPONENT::value_type(L"PlayerGround", pComponent));

	return S_OK;
}

CPlayerGround* CPlayerGround::Create(LPDIRECT3DDEVICE9 pDevice)
{	
	CPlayerGround*		pGround = new CPlayerGround(pDevice);
...
void CPlayerGround::Update(void)
{
...
void CPlayerGround::Render(void)
{
...
void CPlayerGround::Release(void)
{

}

void CPlayerGround::SetPos(float fX, float fY, float fZ)
{
	m_pInfo->m_vPos.x = fX;
	m_pInfo->m_vPos.x = fY;
	m_pInfo->m_vPos.x = fZ;
