...
CHead::CHead(LPDIRECT3DDEVICE9 pDevice)
: CCubeObject(pDevice)
{
}

CHead::CHead(LPDIRECT3DDEVICE9 pDevice, UNITTYPE eUnitType)
: CCubeObject(pDevice)
...
HRESULT CHead::AddComponent(void)
{
	...
	case UT_PLAYER_ESILDUR:
		...
		pComponent = m_pResourcesMgr->CloneResource(ENGINE::RESOURCE_DYNAMIC, L"Texture EsildurFace");
		m_pTexture = dynamic_cast<ENGINE::CTexture*>(pComponent);
		NULL_CHECK_RETURN(m_pTexture, E_FAIL);
		m_mapComponent.insert(MAPCOMPONENT::value_type(L"EsildurFace", pComponent));
		break;
	...
	case UT_PLAYER_ELOND:
		...
		pComponent = m_pResourcesMgr->CloneResource(ENGINE::RESOURCE_DYNAMIC, L"Texture ElondFace");
		m_pTexture = dynamic_cast<ENGINE::CTexture*>(pComponent);
		NULL_CHECK_RETURN(m_pTexture, E_FAIL);
		m_mapComponent.insert(MAPCOMPONENT::value_type(L"EsildurFace", pComponent));
		break;
	...
	case UT_ELF_SOLDIER:
		...
		pComponent = m_pResourcesMgr->CloneResource(ENGINE::RESOURCE_DYNAMIC, L"Texture ElfFace");
		m_pTexture = dynamic_cast<ENGINE::CTexture*>(pComponent);
		NULL_CHECK_RETURN(m_pTexture, E_FAIL);
		m_mapComponent.insert(MAPCOMPONENT::value_type(L"ElfFace", pComponent));
		break;
...
void CHead::Release(void)
{

}

CHead* CHead::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CHead* pHead = new CHead(pDevice);

	if(FAILED(pHead->Initialize()))
		ENGINE::Safe_Delete(pHead);

	return pHead;
}

CHead* CHead::Create(LPDIRECT3DDEVICE9 pDevice, UNITTYPE eUnitType)
{
	CHead* pHead = new CHead(pDevice, eUnitType);
