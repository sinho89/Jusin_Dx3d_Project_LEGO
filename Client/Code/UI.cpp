...
HRESULT CUI::AddComponent(void)
{
	...
	// Texture
	pComponent = m_pResourcesMgr->CloneResource(ENGINE::RESOURCE_DYNAMIC, L"Texture PlayerFace");
...
{
	CUI*		pUI = new CUI(pDevice);

	if(FAILED(pUI->Initialize()))
		ENGINE::Safe_Delete(pUI);
...
void CUI::SetPos(float fX, float fY, float fZ)
{
	m_pInfo->m_vPos.x = fX;
	m_pInfo->m_vPos.x = fY;
	m_pInfo->m_vPos.x = fZ;
