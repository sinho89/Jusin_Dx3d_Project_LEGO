...
CHitParticle::CHitParticle(LPDIRECT3DDEVICE9 pDevice, D3DXVECTOR3 _Pos)
...
CHitParticle* CHitParticle::Create(LPDIRECT3DDEVICE9 pDevice, D3DXVECTOR3 _Pos)
{
	CHitParticle*		pHitParticle = new CHitParticle(pDevice, _Pos);

	if(FAILED(pHitParticle->Initialize()))
		ENGINE::Safe_Delete(pHitParticle);

	return pHitParticle;
}

void CHitParticle::Update(void)
{
	if(!m_bAni)
		return;
...
D3DXMATRIX* CHitParticle::GetWorldMatrix(void)
{
	return NULL;
}

D3DXMATRIX* CHitParticle::GetViewMatrix(void)
{
	return NULL;
}

D3DXMATRIX* CHitParticle::GetProjMatrix(void)
{
	return NULL;
}

LPDIRECT3DDEVICE9* CHitParticle::GetDevice(void)
{
	return NULL;
}

void CHitParticle::SetAni(BOOL bAni/*=TRUE*/)
{
	m_bAni = bAni;
...
void CHitParticle::SetAvgTime(FLOAT fTime)
{
	m_fTimeAvg = fTime;
}

void CHitParticle::SetPos(D3DXVECTOR3 _Pos)
{
	m_vTargetPos = _Pos;
}

HRESULT CHitParticle::Initialize(void)
{
	m_PrtN	= 150;
...
void CHitParticle::SetPart(int nIdx)
{
	...
	fColor	*=0.01f;
	pPrt->dColor	=	D3DXCOLOR(1, 0, 0, fColor);


	// 입자의 표현 요소
	pPrt->PrsW = 0.5f + rand()%10;
	pPrt->PrsW *= 0.02f;

	pPrt->PrsH = 0.5f + rand()%10;
	pPrt->PrsH *= 0.02f;
}
